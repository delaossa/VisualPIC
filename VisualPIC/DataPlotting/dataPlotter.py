...
class DataPlotter:
    ...
    def MakePlot(self, figure, subplotList, rows, columns, timeStep=None):
        ...
        figure.subplots_adjust(
            hspace=.3, top=.93, bottom=.09, right=.93, left=.09)
        for subplot in subplotList:
            if subplot != None:
                # create axes
                if subplot.GetAxesDimension() == "3D":
                    ax = figure.add_subplot(
                        rows, columns, subplot.GetPosition(), projection='3d')
                else:
                    ax = figure.add_subplot(rows, columns,
                                            subplot.GetPosition())
                # make plot on axes
                self.PlotFromDataType[subplot.GetDataType()](
                    figure, ax, subplot, rows, columns, timeStep)
    ...
    def MakeFieldPlot(self, figure, ax, subplot, rows, columns, timeStep):
        ...
        for field in subplot.GetFieldsToPlotWithDimension("2D"):
            ...
            field_cmap = self.colorMapsCollection.GetColorMap(
                field.GetProperty("cMap"))
            scale = field.GetProperty("minVal"), field.GetProperty("maxVal")
            autoScale = field.GetProperty("autoScale")
            # make plot
            im = self.plotTypes["Field"][field.GetProperty("plotType")](
                ax, plotData, field_cmap, scale, autoScale)
            ...
            cbWidth = 0.015
            cbHeight = (pos2[3] -
                        (num2DFields - 1) * self.cbSpacing) / num2DFields
            cbX = pos2[0] + pos2[2] + 0.02
            cbY = pos2[1] + i * (cbHeight + self.cbSpacing)
            cbAxes = figure.add_axes([cbX, cbY, cbWidth, cbHeight])
            cbar = figure.colorbar(
                im, cax=cbAxes, cmap=field_cmap, drawedges=False)
            cbar.solids.set_edgecolor("face")
            cbar.set_label(
                label=r'${:}$'.format(units),
                size=subplot.GetColorBarProperty("FontSize"))
            i += 1
            # label axes
            ax.xaxis.set_major_locator(LinearLocator(5))
            ax.set_xlabel(
                subplot.GetAxisProperty("x", "LabelText") + " $[" +
                subplot.GetAxisProperty("x", "Units") + "]$",
                fontsize=subplot.GetAxisProperty("x", "LabelFontSize"))
            ax.set_ylabel(
                subplot.GetAxisProperty("y", "LabelText") + " $[" +
                subplot.GetAxisProperty("y", "Units") + "]$",
                fontsize=subplot.GetAxisProperty("y", "LabelFontSize"))
            ax.set_title(
                subplot.GetTitleProperty("Text"),
                fontsize=subplot.GetTitleProperty("FontSize"))
        ...
        for field in subplot.GetFieldsToPlotWithDimension("1D"):
            ...
            if num2DFields > 0:
                ...
                axis1D = ax
            plotData = field.GetData(timeStep)
            self.plotTypes["Field"][field.GetProperty("plotType")](axis1D,
                                                                   plotData)
            if num2DFields > 0:
                label = r'${:}$'.format(units),
                axis1D.set_ylabel(label)
            else:
    ...
    def MakeAxisDataPlot(self, figure, ax, subplot, rows, columns, timeStep):
        ...
        if "weight" in axisData:
            plotData["weight"] = axisData["weight"].GetDataSetPlotData(
                timeStep)
        cMap = self.colorMapsCollection.GetColorMap(
            subplot.GetPlotProperty("CMap"))
        # make plot
        im = self.plotTypes["Raw"][subplot.GetPlotProperty("PlotType")](
            ax, plotData, cMap)
        # change plot size to make room for the colorBar
        pos1 = ax.get_position()
        pos2 = [pos1.x0, pos1.y0, pos1.width - 0.1, pos1.height]
        ...
        if "weight" in axisData:
            ...
            cbar.solids.set_edgecolor("face")
            cbar.set_label(
                label="$" + axisData["weight"].GetProperty("dataSetUnits") +
                "$",
                size=subplot.GetColorBarProperty("FontSize"))
        # label axes
        ax.xaxis.set_major_locator(LinearLocator(5))
    ...
    def MakeRawEvolutionDataPlot(self, figure, ax, subplot, rows, columns,
                                 timeStep):
        ax.hold(False)
        ...
        for particleData in allPaticlesData:
            ...
            # make plot
            im = self.plotTypes["RawEvolution"][subplot.GetAxesDimension()](
                ax, plotData, particleData["plotStyle"])
            ax.hold(True)
        # label axes
        ax.xaxis.set_major_locator(LinearLocator(5))
        ax.set_xlabel(
    ...
    def MakeImagePlot(self, ax, plotData, cMap, scale, autoScale):
        ...
        axesLimits = [min(xAxisData), max(xAxisData), min(yAxisData), max(
            yAxisData)]
        return ax.imshow(
            fieldData,
            extent=axesLimits,
    ...
    def MakeHistogramPlot(self, ax, plotData, cMap):
        ...
        H, xedges, yedges = np.histogram2d(
            xValues, yValues, bins=80, weights=weightValues)
    ...
    def MakeScatterPlot(self, ax, plotData, cMap):
        ...
        if "weight" in plotData:
            weightValues = plotData["weight"]
            return ax.scatter(
                xValues, yValues, c=weightValues, cmap=cMap, linewidths=0)
        else:
    ...
    def Make3DScatterPlot(self, ax, plotData, cMap):
        ...
        if "weight" in plotData:
            ...
            return ax.scatter(
                xValues, yValues, zValues, cmap=cMap, linewidths=0)
