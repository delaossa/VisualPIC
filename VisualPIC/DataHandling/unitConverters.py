...
class GeneralUnitConverter:
    def __init__(self):
        ...
        self.e = 1.60217733e-19 #C
        self.m_e = 9.1093897e31 #kg
        self.eps_0 = 8.854187817e-12 #As/(Vm)
...
class HiPACEUnitConverter(GeneralUnitConverter):
    ...
    def SetNormalizationFactor(self, value):
        self.normalizationFactor = value * 1e6
        self.normalizationFactorIsSet = True
        self.w_p = math.sqrt(self.n_p * (self.e)**2 / (self.m_e * self.eps_0)) #plasma freq (1/s)
        self.s_d = self.c / self.w_p  #skin depth (m)
        self.E0 = self.c * self.m_e * self.w_p / self.e # cold non-relativistic field in V/m
    ...
    def GetDataISUnits(self, dataElement):
        ...
        units_m = ["x1", "x2", "x3"]
        units_imp = ["p1", "p2", "p3"]
        units_field = ["Ez", "ExmBy", "EypBx"]

        if dataElementName in units_m:
            return "m"
        elif dataElementName in units_imp:
            return "kg*m/s"
        elif dataElementName in units_field:
            return "V/m"
        elif dataElementName == "q":
            return "C/m^2"
        elif dataElementName == "ene":
            return "J"
        elif dataElementName == "q":
            return "C"
